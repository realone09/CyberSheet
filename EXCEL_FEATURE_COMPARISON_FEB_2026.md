...
| **Data Types** | Advanced | **80–85%** ⬆️ | **v2.2-token-layer (Feb 18):** Core types complete (text, number, date, percentage, currency, boolean, error). **Entity infrastructure complete:** Field access tokenization (`A1.Price`, `A1.Stock.Price`), Excel-compatible null semantics, sequential evaluation with type checking. **Week 3 Phase 1:** 84 tests passing (entity tokenization, null coercion), infrastructure ready for linked data types. **Missing:** External API integration for stocks/geography data sources, data type provider system, refresh mechanisms. **Infrastructure ready, data sources pending.** | ⚠️ Quite possible (with external APIs) | Low (15–20%) | **Average** |
